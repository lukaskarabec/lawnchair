package cz.appkazdarma.aiasistent.domain.use_case.get_gemini_prompt
...
import cz.appkazdarma.aiasistent.domain.use_case.get_notifications.GetNotificationsUseCase
import cz.appkazdarma.aiasistent.domain.use_case.get_user_location.GetUserLocationUseCase
import cz.appkazdarma.aiasistent.domain.use_case.get_weather_forecast.GetWeatherForecastUseCase
...
class GetGeminiPromptUseCase @Inject constructor(
    ...
    suspend operator fun invoke(prompt: String? = null, addContext: Boolean = true) : Pair<String, List<Content>> {
        ...
        val systemContext = buildString {
            appendLine("--------------------------------------------------")
            appendLine("SYSTEM PROMPT: ${Constants.getSystemPrompt(context)}")
            appendLine("")
...
//            appendLine("")
        ...
        val promptString = prompt ?: Constants.getDefaultPrompt(context)
